...
\item Construction of CPM models (DAGs with their rates/probabilities and MHN models) and simulation of synthetic data from them.
  ...
  \item Analysis of data simulated under one model with methods that have different models (e.g., data simulated from CBN analyzed with OT and OncoBN). \textbf{Examples \qref{sec:model-with-or} and \qref{sec:model-with-and}}.

  \item Analysis of data simulated under one model with manual modification of specific genotype frequencies prior to analyses (e.g., data simulated under CBN but where, prior to analysis, we remove all observations with the WT genotype and the genotype with all loci mutated). \textbf{Example \qref{CBN-no-WT}}. 
...
See additional documentation in \url{https://rdiaz02.github.io/EvAM-Tools}. In particular, additional details about CPMs, error models, predicting genotype frequencies, generating random evam models, and a FAQ is available from \url{https://rdiaz02.github.io/EvAM-Tools/pdfs/evamtools_methods_details_faq.pdf}.
...
\includegraphics[width=0.6\textwidth]{import_00B.png}
...
\vspace*{15pt}
 ...
 We click ``Run evamtools'' and the output is shown in about 30 to 50 seconds\footnote{In this example, changing the setting for the number of MCMC iterations of H-ESBCN from 100000 to 500000 is responsible for increasing the total running time from about 30 to about 50 seconds.}. For easier display of the figures in this document, we select first three of the methods to show, by clicking, on the left menu, under ``Customize the visualization'':
...
This is the output from the first three methods (CBN, OT, OncoBN):
 ...
 That OT and OncoBN give identical results is not surprising since OncoBN is using the disjunctive (OR relationships) model and has not found any disjunctive pattern.  We can run OncoBN using the conjunctive model. Go back to ``User input'' and click on ``Advanced options and CPMs to run'' and set, for ``OncoBN options'', the ``Model'' to ``Conjunctive'':

 \vspace*{5pt}
\includegraphics[width=0.5\textwidth]{onco_conjunct}
...
\includegraphics[width=.6\textwidth]{out_brca_1_ob_conj}
...
Thus, CBN seems to find support in the data for conjunctive dependencies that neither OncoBN (run using the ``Conjunctive'' model) nor H-ESBCN find.
 ...
 \makebox[\textwidth][c]{
   \includegraphics[width=1.2\textwidth]{out_brca_data_1}
   }
\vspace*{15pt}
...
\makebox[\textwidth][c]{
 \includegraphics[width=1.2\textwidth]{out_brca_data_2}
}
\vspace*{15pt}
...
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{brca_x10}
}
...
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{out_brca_x10_1}
}

%% zz: same with 500000 iterations
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{out_brca_x10_2}
}
...
This is the output from the web app (run takes a little over one minute):


\vspace*{5pt}

\makebox[\textwidth][c]{
 \includegraphics[width=1.31\textwidth]{ov_out_1}
}
...
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{ov_out_2_A}
}
...
\includegraphics[width=0.5\textwidth]{ov_out_dbn_c}
...
\vspace*{5pt}
\includegraphics[width=0.9\textwidth]{ov_out_2_500K_pred_gen.png}

\clearpage

%% \includegraphics[width=0.6\textwidth]{ov_out_2_B_200K_4321_pred_gen.png}

\vspace*{2pt}
\includegraphics[width=0.9\textwidth]{ov_out_2_B_500_12_pred_gen.png}

\vspace*{13pt}
%% \includegraphics[width=0.6\textwidth]{ov_out_2_another_or.png}
% \includegraphics[width=0.6\textwidth]{ov_out_2_xor.png}



We can increase the number of MCMC iterations. When using 1000000 most runs tend to give this model:

\includegraphics[width=0.9\textwidth]{ov_out_2_B_1M_NA_pred_gen.png}
...
\item Go to the  ``Rename the data'' box, and add the settings you will use to the name of the data; for example, for 500000 MCMC iterations with seed 14 for H-ESBCN enter \texttt{ov\_500K\_s14} in ``Give your data a name'', and \textbf{click on ``Rename the data''}.\label{reps1}
  ...
  \includegraphics[width=0.9\textwidth]{rename_ov_500K_s14}
...
From the ``User input'' tab we select the ``Enter genotype frequencies manually'':
  ...
  \includegraphics[width=0.2\textwidth]{manual-1}
...
\vspace*{5pt}
 ...
 \vspace*{5pt}
  ...
  \includegraphics[width=0.5\textwidth]{manual-4}

\flushleft and then on ``Add genotype''. We next click on B and C in ``Mutations'' putting a 12 in Counts. We finally add 20 individuals with the ``A, D'' genotype (steps as before: click on A and D for mutations, and put a 20 in counts). After these steps, we can see the genotype composition as both a histogram and table:

   \vspace*{5pt}

  \includegraphics[width=0.8\textwidth]{manual-5}
  ...
  \includegraphics[width=0.5\textwidth]{manual-5b}
  ...
  \includegraphics[width=0.5\textwidth]{manual-5c}
  ...
  \includegraphics[width=1.0\textwidth]{manual-6}
  ...
  \includegraphics[width=1.0\textwidth]{manual-7}
...
\item How do different methods perform when data has been generated under the assumptions of another model?
\end{itemize}
...
\includegraphics[width=0.5\textwidth]{ocb1.png}

\vspace*{5pt}

\flushleft and you can see that the last column of the DAG table is now called ``theta'':

\vspace*{5pt}

\includegraphics[width=0.5\textwidth]{ocb2.png}
...
\includegraphics[width=0.5\textwidth]{ocb3.png}
...
\includegraphics[width=0.5\textwidth]{ocb4.png}
...
\includegraphics[width=0.3\textwidth]{mhne1.png}
...
\includegraphics[width=0.5\textwidth]{mhne2.png}

Now, we click on ``Generate data from MHN model'':
\vspace*{5pt}

\includegraphics[width=0.5\textwidth]{mhne3.png}
...
\includegraphics[width=0.5\textwidth]{mhne4.png}
...
\includegraphics[width=0.5\textwidth]{mhne5.png}
...
\includegraphics[width=0.5\textwidth]{mhne6.png}

Notice how genotype B is more common than either A or C, but now whenever there is B it frequently in combination with C (much  larger frequencies of genotypes B,C and A,B,C).
...
\includegraphics[width=0.5\textwidth]{mhne7.png}
...
Here, we will simulate data under a model that includes both AND, OR, and XOR relationships (e.g., H-ESBCN). EvAM-Tools, in its web app, already includes such a model for five genes:
 ...
 \makebox[\textwidth][c]{
   \includegraphics[width=1.2\textwidth]{and_or_xor}
   }
...
\vspace*{5pt}
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{and_or_xor-b}
}

\flushleft (Of course, the actual simulated data you are likely to obtain will differ from this one).
...
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{and_or_xor_out1-tp}
}

\vspace*{15pt}

\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{and_or_xor_out1-pg}
 }
\clearpage

\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{and_or_xor_out2-tp}
}

\vspace*{15pt}
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{and_or_xor_out2-pg}
}
...
\includegraphics[width=0.65\textwidth]{and_or_xor_out3}
...
\includegraphics[width=0.8\textwidth]{sample_genotypes}
...
\includegraphics[width=0.8\textwidth]{sample_genotypes_out}
...
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{sample_genotypes_out_methods}
}
...
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{and}
}
...
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{and__out_1}
}

\makebox[\textwidth][c]{
\includegraphics[width=1.1\textwidth]{and__out_2}
}
\vspace*{15pt}
...
\includegraphics[width=0.65\textwidth]{and__out_3}
...
\vspace*{10pt}
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{cbn_no_wt_1.png}
}
\vspace*{10pt}
...
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{cbn_no_wt_2.png}
}
...
\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{cbn_no_wt_3.png}
}
...
We could, instead, eliminate all the observations with the four mutations (e.g., maybe genotypes with  three driver mutations are already generally lethal to the organism?). We go to ``User input'', select, on the left, ``data\_original'', rename it (``no\_all\_mut''), and set to 0 the A,B,C,D genotype.

\vspace*{10pt}

\makebox[\textwidth][c]{
\includegraphics[width=1.2\textwidth]{cbn_no_wt_4.png}
}
