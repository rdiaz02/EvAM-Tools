...
// RDU kludge to keep separate lambdas; the initial, and then the iterative
void write_lambda_iter(char* filestem, double* lambda, const int n)
{
	int i;

	char suffix[15] = ".lambda_i";
	char *filename = (char *) calloc(strlen(filestem) + strlen(suffix) + 1, sizeof(char));
	strcat(filename, filestem);
	strcat(filename, suffix);
...
// RDU kludge to write likelihood
void write_likelihood_iter(char* filestem, double loglik_new)
{
	int i;

	char suffix[15] = ".logliknew";
	char *filename = (char *) calloc(strlen(filestem) + strlen(suffix) + 1, sizeof(char));
	strcat(filename, filestem);
	strcat(filename, suffix);
...
double EM_epsilon(model* M, data* D, int N_u, double* Prob, double** condprob, double* epsilon)
{
	...
	while(iter < 10 && (iter < 2 || fabs(delta_epsilon)/ *epsilon > 1e-4 ) )
	{
	  ...
	  // RDU: alert to it, since possible non-identifiability
	  if ((iter > 5) && (loglik_new < loglik))  // should never happen!
            {
              fprintf(stderr, "Former Error in EM_epsilon: Likelihood is decreasing!\n");
              fprintf(stderr, "     checking if within EM_ACCURACY now. Values: \n");
              fprintf(stderr, "  New: ");
              fprintf(stderr,"%.17g", loglik_new);
              fprintf(stderr, "  Previous: ");
              fprintf(stderr,"%.17g", loglik);
              fprintf(stderr, "  Difference old - new: ");
              fprintf(stderr,"%.17g", loglik - loglik_new);
              fprintf(stderr, "\n");

            }
	  // RDU but do not exit unless this condition
	  if ((iter > 5) && ((loglik_new + EM_ACCURACY) < loglik))  // should never happen!
		{
			fprintf(stderr, "Error in EM_epsilon: Likelihood is decreasing!\n");
			exit(1);
		}
...
double local_search(model* M, data* D, int N_u, double* lambda, double* epsilon, double loglik, double T, int N_iter, char* filestem, int l_flag, int w_flag)
{
	...
	while (iter <= N_iter)
	{
		...
		if (w_flag){
			...
			// RDU
			// printf("--- writing lambda \n");
			write_lambda_iter(filestem, lambda, M->n);
			write_likelihood_iter(filestem, loglik_new);
		}
