...
test_that("Minimal examples of HyperTraPS and BML", {
  ...
  expect_true(exists("BML_primary_output", out3))
  expect_true(ncol(out3$BML_primary_output$bootstrap$EdgeProbabilities) == 7)


  out3_nb <- evam(dB_c1,
                  methods = c("BML"),
                  bml_opts = list(ntree = 3, rep = 0))
  expect_true(exists("BML_primary_output", out3_nb))
  expect_true(!exists("BML_primary_output$bootstrap", out3_nb))
  ...
  expect_true(exists("lik.traces", out_h_1$HyperTraPS_primary_output))
  expect_true(out_h_1$HyperTraPS_primary_output$L == 5)

  ## And they can be plotted
  hypertrapsct::plotHypercube.influencegraph(out_h_3$HyperTraPS_primary_output)
  hypertrapsct::plotHypercube.motifs(out_h_1$HyperTraPS_primary_output)
