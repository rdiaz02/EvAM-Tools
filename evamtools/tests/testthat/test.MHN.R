local({
  ...
  test_that("Diagonals of MHN trm", {
    ...
    ## Q_ii = - \Sum Q_ji
    ## paranoid check
    expect_equal(rep(0, 16), rowSums(trm1),
  ...
  approach", {
    ...
    expect_equal(trans_mat_comp_exp,
  ...
  test_that("Verify code to obtain transition rate matrix", {
...
local({
  ...
  setwd("../../inst/miscell/MHN_data/")
  
  ## Example of calls from original Schill code
  ##Cancer Progression Data----------------
  test_that("Workign with breast cancer", {

    
    Dat <- readRDS(file = "BreastCancer.rds") 
  ...
  test_that("Test do_MHN and do_MHN with BreastsCancer data", {
    DatBS <- readRDS(file = "BreastCancer.rds") [1:50, 1:4]
    DatCC <- readRDS(file = "ColorectalCancer.rds")[1:300, 1:9] 
    DatRCC <- readRDS(file = "RenalCellCarcinoma.rds")[, 1:9]
    ...
    for(data in datasets){
      ...
      expect_identical(mhn0$transitionRateMatrix, as.matrix(mhnSM$transitionRateMatrix))
    ...
    Dat11 <- readRDS(file = "Glioblastoma.rds")
...
test_that("we are using the indices of theta
correctly 1", {
  local({
