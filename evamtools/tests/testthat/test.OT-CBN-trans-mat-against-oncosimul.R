...
OncoSimulR's based cpm_to_trans_mat_oncosimul.", {
    ...
    run_test_for_dataset1 <- function(data) {
        expect_equal(
            ...
            reorder_trans_mat(cpm2tm(list(edges = data))$trans_mat_genots),
            ignore_attr = FALSE)
...
##############################
## Using cpm2F2tm with scaling (a = 0.02) as per the new cpm_to_fitness_genots
## To show OK, we use here 0.02, the default for OT

test_that("Testing cpm2tm by comparing with
OncoSimulR's based cpm_to_trans_mat_oncosimul. a = 0.006/0.02", {
    ## Recall cpm2F2tm <- cpm_to_trans_mat_oncosimul
    ...
    run_test_for_dataset1 <- function(data) {
        expect_equal(
            reorder_trans_mat(cpm2F2tm(data, max_f = NULL, a = 0.02)$transition_matrix),
            reorder_trans_mat(cpm2tm(list(edges = data))$trans_mat_genots),
            ignore_attr = FALSE)

        ## Do not run next test if fitness is absurdly large as scaling will fail
        ## as it should.
        max_fitness <- max(cpm2F2tm(data, max_f = NULL, a = 0.02)$accessible_genotypes)
        if(max_fitness < 1e10) {
            maxff <- sample(c(2, 3, 3.5, 3.8, 4, 5, 8), size = 1)
            expect_equal(as.matrix(cpm2F2tm(data, max_f = NULL, a = 0.02)$transition_matrix),
                         as.matrix(cpm2F2tm(data, max_f = maxff, a = 0.02)$transition_matrix))
        }
    }

    ## this takes the object, not the object$edges
    run_test_for_dataset <- function(data){
        expect_equal(reorder_trans_mat(cpm2F2tm(data$edges, max_f = NULL, a = 0.02)$transition_matrix),
                     reorder_trans_mat(cpm2tm(
                         data)$trans_mat_genots),
                     ignore_attr = FALSE)
        maxff <- sample(c(2, 3, 3.5, 3.8, 4, 5, 8), size = 1)
        expect_equal(as.matrix(cpm2F2tm(data$edges, max_f = NULL, a = 0.02)$transition_matrix),
                     as.matrix(cpm2F2tm(data$edges, max_f = maxff, a = 0.02)$transition_matrix))
    }
