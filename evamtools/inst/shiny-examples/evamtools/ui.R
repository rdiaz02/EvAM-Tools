                            ...
                            tags$div(id="all_advanced_options",
                                     tags$div(class="inlin",
                                           ...
                                           inputWithHelperUnbold(numericInput, "HyperTraPS_walkers", "walkers ",
                                                                 "(number of walkers): ", "Look at the likelihood trace: if the working likelihood (in red) and the two recalculated estimates (dashed black) disagree, more walkers should be used.  See details in section 'Assessing performance' in the README of HyperTraPS-CT repo.", 200, min=0),
                                           ...
                                           inputWithHelperUnbold(numericInput, "HyperTraPS_kernel", "kernel ",
                                                                 "(perturbation kernel): ", "The width of the MCMC kernel. The default of 5 should work well in most cases. Long periods of stasis on the likelihood trace (a horizontal line) indicate the MCMC acceptance rate is too low; in this case, use a smaller kernel. See details in section 'Assessing performance' in the README of HyperTraPS-CT repo.", 5, min=0),
                                           ...
                                           inputWithHelperUnbold(selectInput, "HyperTraPS_apm", "apm ",
                                                                 "(use APM; No = 0, Yes = 1): ", "Auxiliary pseudo-marginal MCMC (see Greenbury et al. 2020). Attempts to account for the random nature of the likelihood sampler by conditioning on random seed, which also varies throughout the MCMC run. Computationally more involved, but may prevent chains getting stuck.",
                                                                 c(0, 1), selected = 0),
                                           inputWithHelperUnbold(selectInput, "HyperTraPS_sa", "sa    ",
                                                                 "(use SA: 0/1):", "Simulated annealing. Rather than running MCMC, optimises parameter set against a decreasing temperature profile (inverse quarter power of the step). Will give a final point parameter estimate rather than a posterior distribution.", c(0, 1), selected = 0),
                                           inputWithHelperUnbold(selectInput, "HyperTraPS_sgd", "sgd ",
                                                                 "(use SGD 0/1):", "Stochastic gradient descent. Rather than running MCMC, estimate likelihood gradient at each step and use this (scaled) to propose the next move. Will give a final point parameter estimate rather than a posterior distribution.", c(0, 1), selected = 0),
                                           inputWithHelperUnbold(numericInput, "HyperTraPS_sgd_scale", "sgd_scale ",
                                                                 "(scaling factor for gradient in SGD):", "If using SGD, how much to scale the local likelihood gradient to determine the next parameter step.", 0.01),
                                           inputWithHelperUnbold(selectInput, "HyperTraPS_pli", "pli ",
                                                                 "(use PLI 0/1):", "Phenotype landscape inference (see Williams et al. 2014). PLI applies no bias to the sampling walkers, reducing efficiency. Included for back-compatibility.", c(0, 1), selected = 0),
                                           inputWithHelperUnbold(selectInput, "HyperTraPS_samplegap", "samplegap ",
                                                                 "(gap between posterior samples): ", "Number of MCMC steps between parameter samples that are treated as posterior samples. Higher values guard against correlated sampling. As a rule: 1e3 (>1e4 steps), 1e2 (>1e3 steps), or 1. The default of 10 seems to generally be enough, but if the likelihood traces suggest that the chains aren't mixing well (i.e., if there seems to be large autocorrelation), increase the samplegap. samplegaps that are not integer divisors of 10^length can cause problems, so we limit the values here (and samplegaps must be smaller than the chaing length).", c(1000, 500, 200, 100, 50, 20, 10, 1), selected = 10),
                                           inputWithHelperUnbold(numericInput, "HyperTraPS_penalty", "penalty ",
                                                                 "(penalty for per non-zero parameter):", "", 0,
                                                                 min=0, step = 0.01),
                                           inputWithHelperUnbold(numericInput, "HyperTraPS_LASSO", "lasso ",
                                                                 "(LASSO penalty scaling summed absolute parameter values):", "", 0, min=0,
                                                                 step = 0.01),
