                            ...
                            tags$div(id="all_advanced_options",
                                     tags$div(class="inlin",
                                           ...
                                           inputWithHelperUnbold(numericInput, "HyperTraPS_length", "length ",
                                                                 "(inference chain length): ", "The number of steps is 10^length.", 3, min=0, step = 0.1),
                                           ...
                                           inputWithHelperUnbold(selectInput, "HyperTraPS_apm", "apm_type ",
                                           ...
                                           inputWithHelperUnbold(numericInput, "HyperTraPS_samplegap", "samplegap ",
                                                                 "(gap between posterior samples): ", "Number of MCMC steps between parameter samples that are treated as posterior samples. Higher values guard against correlated sampling. As a rule: 1e3 (>1e4 steps), 1e2 (>1e3 steps), or 1. The default of 10 seems to generally be enough, but if the likelihood traces suggest that the chains aren't mixing well (i.e., if there seems to be large autocorrelation), increase the samplegap.  samplegaps must be smaller than the chain length.", 10, min = 1, step = 1),
