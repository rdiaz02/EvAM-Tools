                            ...
                            tags$div(id="all_advanced_options",
                                     tags$div(class="inlin",
                                           ...
                                           inputWithHelperUnbold(numericInput, "HyperTraPS_walkers", "walkers ",
                                                                 "(number of walkers): ", "Look at the likelihood trace: if the working likelihood (in red) and the two recalculated estimates (dashed black) disagree, more walkers should be used.", 200, min=0),
                                           ...
                                           inputWithHelperUnbold(numericInput, "HyperTraPS_kernel", "kernel ",
                                                                 "(perturbation kernel): ", "The width of the MCMC kernel. The default of 5 should work well in most cases; if the MCMC of the acceptance rate is too low, use a smaller value, and if it is too large, use a larger value (though this is hard to tell just from the likelihood trace, so for now, unless the trace shows clearly pathological behavior, use 5).", 5, min=0),
                                           ...
                                           inputWithHelperUnbold(numericInput, "HyperTraPS_samplegap", "samplegap ",
                                                                 "(gap between samples):", "The default of 10 seems to generally be enough, but if the likelihood traces suggest that the chains aren't mixing well (i.e., if there seems to be large autocorrelation), increase the samplegap.", 10, min = -1),
