                            ...
                            tags$div(id="all_advanced_options",
                                     tags$div(class="inlin",
                                           ...
                                           inputWithHelperUnbold(selectInput, "HyperTraPS_samplegap", "samplegap ",
                                                                 "(gap between posterior samples): ", "Number of MCMC steps between parameter samples that are treated as posterior samples. Higher values guard against correlated sampling. As a rule: 1e3 (>1e4 steps), 1e2 (>1e3 steps), or 1. The default of 10 seems to generally be enough, but if the likelihood traces suggest that the chains aren't mixing well (i.e., if there seems to be large autocorrelation), increase the samplegap.  samplegaps must be smaller than the chain length.", c(1000, 500, 200, 100, 50, 20, 10, 1), selected = 10),
